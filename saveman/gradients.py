...
def _pca_gradients(x, ref=None):
    ...
    gm = GradientMaps(n_components=None, approach='pca', kernel='cosine',  # type: ignore
                      alignment=alignment)
...
def create_reference(dataset_dir, ses='ses-01', specifier='base', 
    ...
    display(f'number of matrices to create reference ------------> {len(cmat_files)}')
...
def main():
    ...
    ref_cmat = create_reference(config.connect + suffix, ses='both', 
