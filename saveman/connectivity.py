...
from saveman.ventral_visual_cleanup import clean_cerebellum_visual_bleed
from saveman.utils import get_files, display
...
def _window_length():
    """Number of scans in each window for epochs
    """
    return 96
...
def _split_by_learning(x, include_base=True):
    """Divide rotation scan into baseline, early learning, and late learning"""
    base_window_length = _baseline_window_length()
    window_length = _window_length()
    if include_base:
        baseline = x[:window_length]
        early = x[base_window_length:base_window_length + window_length]
...
def join_sub_cerebellum(data, fname):
    config = Config()
...
def compute_connectivity(timeseries, output_dir, join_sub_cereb=True, float_fmt='%1.8f', 
                         split='learning', clean_visual_bleed=True):
    ...
    if join_sub_cereb:
        ...
        data = data.iloc[:, 32:]
    if clean_visual_bleed:
        schaefer_centroid_path = pjoin(Config().resources, 
                                       'atlases/Schaefer2018_400Parcels_7Networks_order_FSLMNI152_1mm.Centroid_RAS.csv')
        data, _ = clean_cerebellum_visual_bleed(
        ts_path=data,
        schaefer_centroid_path=schaefer_centroid_path,
        mode="pca",
        z_thresh=-4.0,     # ventral: z < 0
        y_thresh=None,   # optional: posterior occipital only
        dorsal_only=False
        )
    tseries = data.values
...
def connectivity_analysis(input_data, out_dir, ses=None, join_sub_cereb=True, njobs=32):
    ...
    valid_subs = pd.read_csv(pjoin(config.resources, 'subs_list.csv'))
    if ses is not None:
        timeseries = get_files(input_data + f'/*{ses}*.tsv')
    else:
        timeseries = get_files(input_data + '/*.tsv')
    timeseries = [ts for ts in timeseries if any(sub in ts for sub in valid_subs['sub'])]
    # get only task data
...
def main():
    ...
    config = Config()
    connectivity_analysis(config.tseries, config.connect, ses=None, join_sub_cereb=True)
